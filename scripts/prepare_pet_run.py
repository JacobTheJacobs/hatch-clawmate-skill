...
ATLAS = {"columns": 8, "rows": 22, "cell_width": 192, "cell_height": 208}
...
ROWS = [
    ...
    ("coding", 9, 6, "coding loop with compact, identity-preserving work motion"),
    ("communicating", 10, 6, "communicating loop with subtle attached signal or expressive pose"),
    ("monitoring", 11, 6, "monitoring loop with attentive scanning or status-check motion"),
    ("read-file", 12, 6, "read-file loop with focused page/file-reading posture"),
    ("syncing", 13, 6, "syncing loop with calm rhythmic coordination or attached flourish"),
    ("system", 14, 6, "system-state loop showing steady process attention"),
    ("run-tool", 15, 6, "run-tool loop showing deliberate invocation or tool-use motion"),
    ("reaction", 16, 6, "reaction loop with clear compact emotional feedback"),
    ("web-fetch", 17, 6, "web-fetch loop with attentive retrieval pose"),
    ("web-search", 18, 6, "web-search loop with curious scan/search pose"),
    ("working", 19, 6, "working loop with purposeful, non-locomotion activity"),
    ("vibe", 20, 6, "pet-specific personality flourish, compact and attached, never smoking"),
    ("write-to-file", 21, 6, "write-to-file loop with compact save/write action"),
]
...
STATE_REQUIREMENTS = {
    ...
    "coding": [
        "Show compact coding/work motion through the pet pose and any identity-consistent existing prop only.",
        "Do not add detached code glyphs, UI panes, labels, floating punctuation, or oversized new props.",
    ],
    "communicating": [
        "Show communication through expressive pose, head angle, or a tiny attached accent that overlaps or touches the pet.",
        "Do not add speech bubbles, detached signal clouds, loose icons, or text.",
    ],
    "monitoring": [
        "Show focused observation through lean, alert face, gentle scan motion, or identity-consistent prop motion.",
        "Do not add dashboards, detached meters, charts, labels, or UI.",
    ],
    "read-file": [
        "Show reading through attentive pose, eye/head movement, or an identity-consistent existing small prop.",
        "Do not add detached pages, UI documents, labels, or readable text.",
    ],
    "syncing": [
        "Show synchronization through steady rhythmic body/prop motion or a tiny attached flourish.",
        "Do not add detached arrows, rings, halos, bubbles, or floating sync icons.",
    ],
    "system": [
        "Show system-state readiness through composed posture and subtle process motion.",
        "Do not add console windows, warning labels, code snippets, or detached status icons.",
    ],
    "run-tool": [
        "Show deliberate tool invocation through pose or existing accessory interaction.",
        "Do not add oversized tools, detached cursor icons, labels, or UI panels.",
    ],
    "reaction": [
        "Show a compact expressive reaction while keeping the whole pet dominant.",
        "Any small effect must be attached or overlapping, not floating separately.",
    ],
    "web-fetch": [
        "Show retrieval/fetch intent through forward attention or subtle identity-consistent action.",
        "Do not add detached browser panels, URLs, globes, labels, or oversized props.",
    ],
    "web-search": [
        "Show search intent through curious scanning posture or subtle head/eye motion.",
        "Do not add magnifying glasses, browser windows, detached question marks, or text unless already core to the base identity.",
    ],
    "working": [
        "Show productive work motion without directional travel.",
        "Do not reuse literal running locomotion; avoid detached work symbols or UI.",
    ],
    "vibe": [
        "Vibe is a pet-specific personality flourish, never a smoking row.",
        "Use a compact attached moment such as a tiny steam puff touching the pet, sparkle overlapping the silhouette, relaxed sway, accessory flourish, flower sniff, bubble puff, or soft hard-edged aura wisp that stays attached.",
        "Keep the pet dominant and the flourish secondary. No detached effect clouds, oversized props, labels, or UI.",
    ],
    "write-to-file": [
        "Show compact write/save action through pose or identity-consistent prop motion.",
        "Do not add detached file icons, floating documents, labels, or UI panels.",
    ],
}
