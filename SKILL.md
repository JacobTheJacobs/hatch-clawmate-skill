...
Normal generation must use Codex-auth image generation first. That means the standard path is the current Codex session's `$imagegen` capability, including Codex-auth execution paths that can attach the run's grounding images when needed. Do not ask for `OPENAI_API_KEY` during a normal Codex run.

Do not call the external Image API directly for the normal path. Let `$imagegen` choose its own built-in-first path and its own CLI fallback rules. If `$imagegen` says a fallback requires confirmation, ask the user before continuing.
...
Hard boundary: do not create, draw, tile, warp, mirror, or synthesize pet visuals with local Python/Pillow scripts, SVG, canvas, HTML/CSS, or other code-native art as a substitute for `$imagegen`. For a normal pet run, expect up to 23 visual generation jobs: 1 base pet plus 22 row-strip jobs. The only exception is `running-left`, which may be derived by mirroring `running-right` only after `running-right` has been generated, visually inspected, and explicitly approved as safe to mirror. If mirroring is not appropriate, generate `running-left` as a normal grounded `$imagegen` row. If those calls are too expensive, blocked, or unavailable, stop and explain the blocker instead of fabricating row strips locally.
...
- `coding`: keep the pet dominant; show compact work motion without detached code glyphs, UI panes, or labels.
- `communicating`: use expressive posture or a tiny attached accent only; no detached speech bubbles, loose signal clouds, or text.
- `monitoring`: show alert attention or subtle scanning posture; no dashboards, meter panels, or labels.
- `read-file`: show focused reading posture; no readable text, detached pages, or UI.
- `syncing`: use steady rhythmic motion or a tiny attached flourish; no detached arrows, rings, or floating sync icons.
- `system`: show composed process-readiness; no console windows, warning labels, or code snippets.
- `run-tool`: show deliberate invocation through pose or identity-consistent accessory motion; no oversized new props or UI.
- `reaction`: keep the reaction compact and whole-body readable; any tiny effect must stay attached or overlapping.
- `web-fetch`: show retrieval intent through pose; no browser panels, URLs, globe icons, or labels.
- `web-search`: show curious searching posture; no detached question marks, browser windows, or labels.
- `working`: show purposeful non-locomotion work motion; do not reuse foot-running.
- `vibe`: this is a pet-specific personality flourish, never smoking. Use a compact attached moment such as a tiny steam puff touching the pet, sparkle overlapping the silhouette, relaxed sway, accessory flourish, flower sniff, bubble puff, or soft hard-edged aura wisp that remains attached. Keep the pet dominant and the flourish secondary.
- `write-to-file`: show compact write/save motion; no detached file icons, labels, or UI panels.
...
`prepare_pet_run.py` also creates 22 row-specific layout guide images under `references/layout-guides/`, one per animation state. Row jobs attach the matching guide as a layout-only input so the model can follow the correct frame count, spacing, centering, and safe padding. Treat these guides as invisible construction references: the generated row strip must not include visible boxes, borders, center marks, labels, guide colors, or the guide background.
...
Fallback order inside `generate_pet_images.py` must be:

1. Codex-auth `$imagegen` using the current Codex session.
2. Codex-auth `codex exec` with `--image` attachments when grounded row inputs are required.
3. External OpenAI Images API only if `OPENAI_API_KEY` exists.
4. If `OPENAI_API_KEY` is missing, do not fail unless both Codex-auth paths failed.
...
`OPENAI_API_KEY` is optional and is only used for the external API fallback after the Codex-auth paths fail. Never ask the user for an API key during a normal Codex-auth run.

## Rules

- Keep `$imagegen` as the primary generation layer.
- Use Codex-auth `$imagegen` first for all normal pet generation.
- Treat `OPENAI_API_KEY` as optional and only for the external API fallback path.
- Never ask for `OPENAI_API_KEY` during a normal Codex-auth run.
...
- Final atlas is PNG or WebP, `1536x4576`, transparent-capable, and based on `192x208` cells.
